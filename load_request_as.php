...
$sql="select * from chandj.work where (is_deleted IS NULL or  is_deleted = 0) and checkstep like '%AS요청%' order by num desc"; 					
		...
		<div class="col-sm-12"> 		
			<div class="d-flex  justify-content-center align-items-center "> 				   
				...
				<a href="../load_request_as.php?header=header">
					<span class="badge bg-info me-1 fs-6">  <?=$title_message?> </span> 				
				</a>  </span> 			
			  <h6> <span id="display_as_su" class="badge bg-info"></span> 
			  </h6>
			</div>			 
		</div>				 
	</div>			
...
<table class="table table-bordered table-hover table-sm">
	...
	?>
	<tr onclick="redirectToView_as('<?= $num ?>', '<?= $tablename ?>')">
...
<script> 
function redirectToView_as(num) {
	popupCenter("./work/write_form.php?mode=view&tablename=work&num=" + num , "공사수주내역", 1850, 900); 	      
}
$(document).ready(function(){	    	
	document.getElementById('display_as_su').textContent = ' <?= $total_row ?>';
}); 
</script> 