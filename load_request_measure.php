...
$sql="select * from chandj.work where (is_deleted IS NULL or  is_deleted = 0) and checkstep like '%실측요청%' order by num desc"; 					
		...
		<div class="col-sm-12"> 		
			<div class="d-flex  justify-content-center align-items-center "> 				   
				...
				<a href="../load_request_measure.php?header=header">
				  ...
				  <h6> <span id="display_measure_su" class="badge bg-primary"></span> 
				  </h6>
			</div>			 
		</div>				 
	</div>			
...
<table class="table table-bordered table-hover table-sm">
	...
	?>
		<tr onclick="redirectToView_measure('<?= $num ?>', '<?= $tablename ?>')">
		   ...
		   <td class="text-center">
				...
				switch ($state_work) {
					...
					default: $state_str = "계약전"; $badge_class = "badge bg-primary"; break;  // 회색
				}
				?>
...
<script> 
function redirectToView_measure(num) {
	popupCenter("./work/write_form.php?mode=view&tablename=work&num=" + num , "공사수주내역", 1850, 900); 	      
}
$(document).ready(function(){	    	
	document.getElementById('display_measure_su').textContent = ' <?= $total_row ?>';
}); 
</script> 
