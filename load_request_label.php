...
$sql="select * from chandj.work where (is_deleted IS NULL or  is_deleted = 0) and checkstep like '%인정라벨부착%' order by num desc"; 					
		...
		<div class="col-sm-12"> 		
			<div class="d-flex  justify-content-center align-items-center "> 				   
				...
				<a href="../load_request_label.php?header=header">
					<span class="badge bg-warning me-1 fs-6">  <?=$title_message?> </span> 				
				</a>  </span> 			
			  <h6> <span id="display_label_su" class="badge bg-warning"></span> 
			  </h6>
			</div>			 
		</div>				 
	</div>			
...
<table class="table table-bordered table-hover table-sm">
	...
	?>
			<tr onclick="redirectToView_label('<?= $num ?>', '<?= $tablename ?>')">
				<td class="text-center"><?= $start_num ?></td>
				<td class="text-center"><?= $regist_day ?></td> 							
				<td class="text-start fw-bold text-dark"><?= $workplacename ?></td>
			   ...
			   <td class="text-center">
					...
					switch ($state_work) {
						...
						case 2: $state_str = "시공중"; $badge_class = "badge bg-warning"; break;  // 파란색														
						case 3: $state_str = "시공완료"; $badge_class = "badge bg-danger"; break;  // 빨간색
						default: $state_str = "계약전"; $badge_class = "badge bg-warning"; break;  // 회색
					}
					?>
				...
				<td class="text-center">
					...
					switch ($cableworkStatus) {									
						case "결선중" : $badge_class = "badge bg-warning"; break;
...
<script> 
function redirectToView_label(num) {
	popupCenter("./work/write_form.php?mode=view&tablename=work&num=" + num , "공사수주내역", 1850, 900); 	      
}
$(document).ready(function(){	    	
	document.getElementById('display_label_su').textContent = ' <?= $total_row ?>';
}); 
</script> 